...
async function generateDiaryForDate(dateStr) {
  ...
  //
  // 2026-08-12：显式列出 scope，把新增的 'recent'（「刚才 · 你最近做过
  // 的事」）摘掉。那一块是给聊天用的——它会告诉枢"你 X 分钟前写下了
  // 今天的日记"，而此刻他**正在写**的就是这一篇；带上它，模型会在正文
  // 里绕回来说"我刚才写了篇日记"，甚至复述自己上一版的开头。内容记忆
  // （前几天写了什么、手记里有什么、时轨第几天）照旧全给，一条不少
  try {
    systemPrompt += await buildLocalMemoryBlock(convoText.slice(0, 400), { scope: ['diary', 'ink', 'chronos'] })
